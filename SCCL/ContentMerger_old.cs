...
namespace TehPers.Stardew.SCCL {
    public class ContentMerger_old {
        ...
        private MethodInfo mergeMethod;

        internal ContentMerger_old() {
            this.mergeMethod = this.GetType().GetMethod("Inject", BindingFlags.Public | BindingFlags.Instance);
        }
        ...
        public void Inject(string assetName, object asset) {
            Originals[assetName] = asset;
            Dirty.Add(assetName);
        }

        public T Merge<T>(string assetName) {
            ModConfig config = ModEntry.INSTANCE.config;

            T asset;
            if (Originals.ContainsKey(assetName)) {
                if (Originals[assetName] is T) asset = (T) Originals[assetName];
                else throw new ArgumentException("Generic type is incompatible with original type", "T");
            } else asset = default(T);

            try {

            } catch (Exception ex) {
                //ModEntry.INSTANCE.Monitor.Log(LogLevel.Error, "An error occured while merging " + assetName);
            }
            return asset;
        }

        public void RefreshAssets() {
            foreach (string asset in Dirty) {
                mergeMethod.MakeGenericMethod(asset.GetType()).Invoke(this, new object[] { asset });
            }
            Dirty.Clear();
        }
