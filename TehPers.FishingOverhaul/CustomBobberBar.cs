...
namespace TehPers.FishingOverhaul {
    public class CustomBobberBar : BobberBar {
        ...
        public CustomBobberBar(Farmer user, int whichFish, float fishSize, bool treasure, int bobber) : base(whichFish, fishSize, treasure, bobber) {
            this.User = user;
            this._origStreak = ModEntry.Instance.Api.GetStreak(user);
            ...
            /* Actual code */
            var config = ModEntry.Instance.MainConfig;
            var traits = ModEntry.Instance.Api.GetFishTraits(whichFish);

            // Check if fish is unaware
            this.Unaware = Game1.random.NextDouble() < ModEntry.Instance.Api.GetUnawareChance(user, whichFish);

            // Applies difficulty modifier, including if fish is unaware
            var difficulty = traits?.Difficulty ?? this._difficulty.Value;
            ...
            if (this.Unaware) {
                ...
                Game1.showGlobalMessage(ModEntry.Translate("text.unaware", ModEntry.Translate("text.percent", 1F - config.UnawareSettings.UnawareMult)));
            }
            ...
            var fishQuality = this._fishQuality.Value;
            ...
            if (this._origStreak >= config.StreakSettings.StreakForIncreasedQuality) this._sparkleText.Value = new SparklingText(Game1.dialogueFont, ModEntry.Translate("text.streak", this._origStreak), Color.Yellow, Color.White);
        }

        public override void update(GameTime time) {
            // Speed warp on catching fish
            var distanceFromCatching = this._distanceFromCatching.Value;
            var delta = distanceFromCatching - this._lastDistanceFromCatching;
            var mult = delta > 0 ? ModEntry.Instance.MainConfig.DifficultySettings.CatchSpeed : ModEntry.Instance.MainConfig.DifficultySettings.DrainSpeed;
            ...
            var treasureCatchLevel = this._treasureCatchLevel.Value;
            delta = treasureCatchLevel - this._lastTreasureCatchLevel;
            mult = delta > 0 ? ModEntry.Instance.MainConfig.DifficultySettings.TreasureCatchSpeed : ModEntry.Instance.MainConfig.DifficultySettings.TreasureDrainSpeed;
            ...
            var perfect = this._perfect.Value;
            var treasure = this._treasure.Value;
            var treasureCaught = this._treasureCaught.Value;

            // Check if still perfect, otherwise apply changes to loot
            if (!this._perfectChanged && !perfect) {
                this._perfectChanged = true;
                this._fishQuality.Value = Math.Min(this._origQuality, ModEntry.Instance.MainConfig.DifficultySettings.PreventGoldOnNormalCatch ? 1 : 2);
                ModEntry.Instance.Api.SetStreak(this.User, 0);
                if (this._origStreak >= ModEntry.Instance.MainConfig.StreakSettings.StreakForIncreasedQuality) {
                    Game1.showGlobalMessage(ModEntry.Translate(treasure ? "text.warnStreak" : "text.lostStreak", this._origStreak));
                }
            }
            ...
            if (!this._treasureChanged && !perfect && treasure && treasureCaught) {
                ...
                var qualityBonus = (int) Math.Floor((double) this._origStreak / ModEntry.Instance.MainConfig.StreakSettings.StreakForIncreasedQuality);
            ...
            if (distanceFromCatching <= 0.0) {
                ...
                this._notifiedFailOrSucceed = true;
                if (this._origStreak >= ModEntry.Instance.MainConfig.StreakSettings.StreakForIncreasedQuality) {
                    Game1.showGlobalMessage(ModEntry.Translate("text.lostStreak", this._origStreak));
                }
            }
                ...
                if (perfect) {
                    ModEntry.Instance.Api.SetStreak(this.User, this._origStreak + 1);
                }
                else if (treasure && treasureCaught) {
                    if (this._origStreak >= ModEntry.Instance.MainConfig.StreakSettings.StreakForIncreasedQuality) Game1.showGlobalMessage(ModEntry.Translate("text.keptStreak", this._origStreak));
                    ModEntry.Instance.Api.SetStreak(this.User, this._origStreak);
                }

                // Invoke fish caught event
                var curFish = this._whichFish.Value;
                var eventArgs = new FishingEventArgs(curFish, this.User, this.User.CurrentTool as FishingRod);
                ModEntry.Instance.Api.OnFishCaught(eventArgs);
        ...
        public override void draw(SpriteBatch b) {
            ...
            if (this._scale.Value >= 1.0) {
                ...
                var position1 = new Vector2(this.xPositionOnScreen + 64, this.yPositionOnScreen + 12 + (int) this._bobberBarPos.Value) + this._barShake.Value + this._everythingShake.Value;
                ...
                var position2 = new Vector2(this.xPositionOnScreen + 64, this.yPositionOnScreen + 12 + (int) this._bobberBarPos.Value + 8) + this._barShake.Value + this._everythingShake.Value;
                ...
                var scale = new Vector2(4f, this._bobberBarHeight.Value - 16);
                const int num7 = 0;
                ...
                var position3 = new Vector2(this.xPositionOnScreen + 64, this.yPositionOnScreen + 12 + (int) this._bobberBarPos.Value + this._bobberBarHeight.Value - 8) + this._barShake.Value + this._everythingShake.Value;
                ...
                // Draw the fish
                var fishPos = new Vector2(this.xPositionOnScreen + 64 + 18, this.yPositionOnScreen + 12 + 24 + this._bobberPosition.Value) + this._fishShake.Value + this._everythingShake.Value;
                if (ModEntry.Instance.MainConfig.ShowFish && !ModEntry.Instance.Api.IsHidden(this._origFish)) {
                    ...
                    var fishSrc = new Rectangle(614 + (this._bossFish.Value ? 20 : 0), 1840, 20, 20);
            ...
            var position = new Vector2(this.xPositionOnScreen + (this._flipBubble.Value ? this.width + 64 + 8 : -200), this.yPositionOnScreen + 192);
