using System;
using System.IO;
using System.Reflection;
using Netcode;
using Newtonsoft.Json;

namespace TehPers.Core.Json {
    internal sealed class NetExposer {
        private readonly MethodInfo unwrap = typeof(NetExposer).GetMethod(nameof(NetExposer.Unwrap), BindingFlags.Public | BindingFlags.Static);

        public Type Type { get; set; }
        public byte[] Data { get; set; }

        [JsonConstructor]
        public NetExposer(Type type, byte[] data) {
            this.Type = type;
            this.Data = data;
        }

        public object Unwrap() {
            return this.unwrap.MakeGenericMethod(this.Type).Invoke(null, new object[] { this.Data });
        }

        public static T Unwrap<T>(byte[] data) where T : AbstractNetSerializable {
            var netField = Activator.CreateInstance<T>();
            using (var stream = new MemoryStream(data)) {
                using (var reader = new BinaryReader(stream)) {
                    netField.ReadFull(reader, new NetVersion());
                }
            }

            return netField;
        }
    }
}