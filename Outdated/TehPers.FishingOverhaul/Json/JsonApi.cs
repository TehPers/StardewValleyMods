...
namespace TehPers.FishingOverhaul.Json
{
    internal class JsonApi : IJsonApi
    {
        ...
        private void AddSmapiConverters(IModHelper helper)
        {
            ...
            {
                _mod.Monitor.Log("Unable to add SMAPI's JSON converters. Some config settings might be confusing!", LogLevel.Error);
            }
        }
        ...
        {
            var fullPath = Path.Combine(this._mod.Helper.DirectoryPath, path);
        ...
        public void Serialize<TModel>(TModel model, Stream outputStream, Action<JsonSerializerSettings> settings, bool minify = false) where TModel : class
        {
            ...
            using (var writer = new DescriptiveJsonWriter(textWriter))
            {
                ...
                // Setup JSON Settings
                var jsonSettings = JsonApi.CloneSettings(this._jsonSettings);
        ...
        {
            var fullPath = Path.Combine(source.Path, path);
        ...
        {
            return new ModContentSource(this._mod.Helper);
        }
